...
SPIClass motor_spi(VSPI);// SPI接口（用于编码器通信）
Cubli::Bsp::CanDriver can; // CAN总线驱动对象

//FreeRTOS信号量
SemaphoreHandle_t motor_speed_semaphore;// 二值信号量：通知CAN发送任务有新数据
SemaphoreHandle_t motor_set_speed_mutex;// 互斥锁：保护速度设定值的线程安全
    ...
    InlineCurrentSense *current_sense)
{
    ...
    motor->motion_downsample = 0.0;//无下采样
    ...
    motor->PID_current_q.limit       = 0.85f;// 输出限制为供电电压的85%
    // Low pass filtering time constant
    motor->LPF_current_q.Tf = 0.002;//2ms低通滤波
    // D轴电流环PID（磁场分量，通常希望为0）
...
void setup()
{
    ...
    //初始化三个编码器
    sensor1.init(&motor_spi);  //正常初始化
    sensor2.init(&motor_spi, true);  //ture=反转方向
    sensor3.init(&motor_spi, true);
    ...
    motor3.target = 0;
     ...
     // 创建FreeRTOS同步对象
    motor_speed_semaphore = xSemaphoreCreateBinary(); //二值信号量
    motor_set_speed_mutex = xSemaphoreCreateMutex();  //互斥锁
    ...
    for (;;)
    {
        if (can.CanGetMotorSpeed(get_set_motor1_speed, get_set_motor2_speed, get_set_motor3_speed, get_motor_trans_mode, control_temp))
        {
            ...
            if (xSemaphoreTake(motor_set_speed_mutex, portMAX_DELAY) == pdTRUE)
            {
                ...
                xSemaphoreGive(motor_set_speed_mutex);   // 释放锁
            }
        }
    }
}
...
float SetSpeedLimit(float set_motor_speed, float motor_speed)
{
    ...
    bool temp_set_spped_is_positive_number = true;  //记录正负号
    if (temp_set_spped < 0)
    {
        temp_set_spped_is_positive_number = false;  // 负数标记
    }

    // 速度在650-700 rad/s之间：逐渐减小输出（软限制）
...
void loop()
{
    //动作：程序先去申请“互斥锁”钥匙。如果 3 毫秒 内拿到了，
    //就赶紧把全局的 set_speed复制一份存到本地的 _temp 变量里。
